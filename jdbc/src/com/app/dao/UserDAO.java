...
//  화면에서 사용자 아이디 전달받음
	public boolean checkId(String userIdentification) {
		...
		try {
			...
			System.out.println("checkId(String) SQL 입력 오류");
			e.printStackTrace();
		} catch (Exception e) {
				e.printStackTrace();
		} finally {
...
//  회원가입
	public void insert(UserVO userVO) {
		...
		try {
			...
			System.out.println("insert(UserVO) SQL 입력 오류");
			e.printStackTrace();
		} catch (Exception e) {
				e.printStackTrace();
		} finally {
...
//  로그인
	public boolean login(String userIdentification, String userPassword) {
		...
		try {
			...
			System.out.println("login(String, String) SQL 입력 오류");
			e.printStackTrace();
		} catch (Exception e) {
				e.printStackTrace();
		} finally {
...
//  회원탈퇴, UPDATE (일반 회원: 0, 탈퇴 회원: 1)
	public void drop() {
		...
		try {
			...
			System.out.println("drop() SQL 입력 오류");
			e.printStackTrace();
		} catch (Exception e) {
				e.printStackTrace();
		} finally {
...
//  아이디 찾기(휴대폰 번호)
	public String selectId(String userPhone) {
		...
		try {
			...
			System.out.println("selectId(String) SQL 입력 오류");
			e.printStackTrace();
		} catch (Exception e) {
				e.printStackTrace();
		} finally {
...
//  비밀번호 변경 (로그인 후 현재 비밀번호, 새로운 비밀번호)
	public void changePassword(String userPassword, String newPassword) {
		...
		try {
			...
			System.out.println("changePassword(String, String) SQL 입력 오류");
			e.printStackTrace();
		} catch (Exception e) {
				e.printStackTrace();
		} finally {
...
//  회원정보 수정(닉네임 수정)
	public void update(UserVO userVO) {
		...
		try {
			...
			System.out.println("update(UserVO) SQL 입력 오류");
			e.printStackTrace();
		} catch (Exception e) {
				e.printStackTrace();
		} finally {
...
//  추천수
	public int getCountOfUserRecommenderId(String userIdentification) {
		int count = 0;
		...
		try {
			...
			System.out.println("getCountOfUserRecommenderId(String) SQL 입력 오류");
			e.printStackTrace();
		} catch (Exception e) {
				e.printStackTrace();
		} finally {
...
//  나를 추천한 사람
	public List<UserVO> getRecommenderId(String userIdentification){
		UserVO userVO = null;
		...
		try {
			...
			System.out.println("getRecommenderId(String) SQL 입력 오류");
			e.printStackTrace();
		} catch (Exception e) {
				e.printStackTrace();
		} finally {
...
//	내가 추천한 사람
	public UserVO getMyRecommenderId() {
		UserVO userVO = null;
		String query = "SELECT USER_ID, USER_IDENTIFICATION, USER_PASSWORD, USER_NAME, USER_PHONE, USER_NICKNAME, USER_EMAIL, USER_ADDRESS, USER_BIRTH, USER_RECOMMENDER_ID, USER_STATUS "
		...
		if(resultSet.next()) {
			...
			System.out.println("getMyRecommenderId() SQL 입력 오류");
			e.printStackTrace();
		} catch (Exception e) {
				e.printStackTrace();
		} finally {
